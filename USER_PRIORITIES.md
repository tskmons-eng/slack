...
- 初期状態は必ず `DRY_RUN=true` にし、本番投稿は管理アクションまたは `settings` シートで明示的に `DRY_RUN=false` に変えた時だけ行う。
- 同じスレッドURLは、`linked_threads` シートと投稿先Slackスレッド内の既存投稿の両方で重複確認して二重投稿しない。
- 処理対象は最終更新から60日以内のスレッドだけにする。
- 車体番号／スレIDの子チャンネル対象は `carmore依頼`、`オールマシンサービス`、`依頼_all`、`電話対応`、`依頼_振込` とし、親は引き続き `依頼_車案件` にする。
...
- GAS/Slack制限対策として、GAS実行時間に近づく前に履歴ページ・root投稿・スレッド返信の途中でも安全に停止し、未確認チャンネルを次回優先する。Slack API 429時は `Retry-After` に従って短く待って再試行する。

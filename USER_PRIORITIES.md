...
- 毎日03:00、10:00、13:00、16:00、20:00付近にGASトリガーで自動実行し、PCを閉じていても動く構成にする。
- 誤投稿より未投稿を優先する。
- `依頼＿ALL` の投稿にロケットリアクションが付いたら、`依頼＿請求書` へ自動投稿する。PDFがあれば `【ファイル名 今日の日付】` と元投稿リンク、PDFがなければ元投稿リンクのみを投稿する。

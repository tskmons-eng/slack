...
投稿本文は `message.text` を通知・検索用のフォールバックとして保持し、Slack blocksがある場合は再投稿用にコピーします。コピー対象は `section`、`header`、`context`、`divider`、`rich_text`、`image` で、ボタンや入力欄などのインタラクティブ部品は転送しません。投稿内に `要約` セクションがあり、`次の対応` や記事内の小見出しを2件以上検出できる場合は、転送先で `目次` を自動挿入します。Slackがblocksを拒否した場合はエラーを記録し、本文だけの投稿にフォールバックします。添付ファイルの再アップロードはv1範囲外です。

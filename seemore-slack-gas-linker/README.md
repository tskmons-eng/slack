...
ローカルPC、VPS、Python、SQLiteは使いません。Google Apps Script、Slack API、Googleスプレッドシートだけで動きます。毎日03:00、10:00、13:00、16:00、20:00付近にGASトリガーで実行するため、PCを閉じていても処理されます。
...
`依頼＿ALL` の投稿または返信に指定リアクション `rocket` が付いている場合、`依頼＿請求書` へ転送します。PDFファイルがある場合は以下の形式で投稿します。
...
- ヘッダー行の作成
- `settings` 初期値の作成
- 毎日03:00、10:00、13:00、16:00、20:00付近に `scheduledMain()` を実行するトリガーの作成
...
| `INVOICE_SOURCE_CHANNEL_NAME` | `依頼＿ALL` | ロケットリアクションを確認するチャンネル名です。 |
...
- `?action=invoice_dryrun`
  - `依頼＿ALL` の直近投稿から、ロケットリアクション付きの転送候補を確認します。PDFがない候補はリンクのみとして扱います。Slackへは投稿しません。
...
以降、毎日03:00、10:00、13:00、16:00、20:00付近の `scheduledMain()` 実行で車案件の紐付けと請求書転送を処理します。
